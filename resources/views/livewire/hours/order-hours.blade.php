@if($order_hours->count() > 0)
    <tr class="bg-gray-50 dark:bg-gray-500 dark:border-gray-800">
        ...
        <td colspan="{{count($period)}}" class="py-2 px-4 dark:bg-gray-800"></td>
    </tr>
    @foreach($order_hours->groupBy('order_id') as $order_hour)
        <tr class="bg-gray-100 dark:bg-gray-900 dark:border-gray-800 border-b">
            ...
            @foreach($period as $day)
                <td class="border-r dark:border-gray-700">
                    @if($order_hour->contains(function ($item) use ($day){ return $item->hour->date == $day->format('Y-m-d'); }))
                        @foreach($order_hour->where(function ($item) use ($day){ return $item->hour->date == $day->format('Y-m-d'); }) as $record)
                                ...
                                >
                                    <input class="w-full" type="text" wire:model="order_hours.{{ $order_hours->search($record) }}.hour.count">
{{--                                    {{ $record->hour->count }}--}}
                                </div>
