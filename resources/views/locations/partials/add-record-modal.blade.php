<form method="post" action="{{ route('locations.store') }}" class="p-6">
    ...
    <div class="space-y-6 max-w-md">
        <div class="mt-6">
            <x-input-label for="add_location" :value="__('Position')"/>
            <x-text-input id="add_location" name="location" type="text" class="mt-1 block w-full" required
                          autofocus/>
            <x-input-error class="mt-2" :messages="$errors->get('location')"/>
        </div>

        <div>
            <x-input-label for="add_date" :value="__('Date')"/>
            <div class="relative mt-1">
                ...
                <input id="add_date" name="date" datepicker
                       type="text"
